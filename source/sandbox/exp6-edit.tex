...
\TODO{
	...
	* \answer{Tip for TAs.  Will be removed from student verstion of the lab.}
	\\
...
%\renewcommand{\item}{*}
	* Assume students don't know anything about torque
		(it's very likely they won't know anything)
		\\
	* Give activity which guides them through torque
	\\
	* Possibly just take from open source tutorial
	\\
	* Ok, open source tutorial requires them to do experiment that we don't
...
We'll measure the forces experienced the biceps muscle.
...
\question{predict how the bicep force change when we increase 
...
it's easy to change one of the parameter values in the left hand column 
of the table above.  \question{Which value is this?  }
Once you've determined what it is, 
devise an experiment to 
\question{determine the effect that increasing that parameter
...
\answer{They should choose a new value for $r$, then re-enact their procedure
	to determine $B(H)$ for this new $r$ value.  It might be difficult for them
	...
	\TODO{ We might want to just tell them to vary $r$. }
\TODO{
%	A few things here:  
%	(1) 
	Subtlety: 
...
%	(2) We probably want to remove $\alpha$ from the chart above if we ask
%	them to figure out which parameter to vary.
}
