%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Setup for todonote comments and otherwise
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\newcommand{\TODO}[2][inline,color=green!40,caption={}]{{ \todo[#1]{#2} }}

\newcommand{\question}[2][blue]{\mycolor{#1}{#2}}
\newcommand{\answer}[2][red]{\mycolor{#1}{#2}}
...
FYI, comments written in second person are directed at Jared (who is revising
the lab), not TAs implementing it.
}
...
\begin{comment}
	% Notes on the desirability of having the students do mini-experiments
	%	on static torque as part of the warmups
}}}
...
\answer{Note to TAs:  These warmups are designed to be 
	completely by students with minimal prior exposure to torque.
	I recommend giving a \textbf{very minimal} introduction to torque before
	having the students complete the warmups,
	give them 10-15 minutes to complete them (let them move on to labeling the
	diagrams if they finish early)
	and then go over the torque warmups as a class.
	During/ after this, give a more thorough introduction to torque.

	Before the students start the warmup, 
	I recommend \textbf{briefly} saying something like the following:  
	``We use torque to talk about how much much an object will
	rotate about a pivot.  The torque will depend on (1) forces applied to the
	object, (2) the distance between the forces and the pivot point, 
	and (3) the angle at which the forces are applied.''

	While going over the warmups, I recommend focusing on
	the fact that orque depends on force and distance (with examples)
	I do not recommend quantifying the dependence on the angle, 
	but rather simply mentioning that it has an effect.
}
...
Hint:  If you're confused where to start, know that the horizontal bar
on the apparatus models the forearm.
...
we need to ensure the biceps muscle scale is correctly calibrated,
as it is common for the scale to become miscalibrated
after several lab sections.
\begin{enumerate}[label={\bf \arabic*.}]
	\item Remove the forearm bar from the apparatus,
		so nothing is hanging from the biceps muscle scale.
	\item Hang a $\smans{1}{kg}$ mass from the biceps muscle scale
		(making sure to use the keeper ring to prevent the mass from falling
		off).

		If the scale does not read $\smans{1}{kg}$,
		you need to recalibrate it.
		Do so by turning the `knurled ring` at the top of the force scale,
		in the direction that increases or decreases the force reading as
		needed, until the scale reads $\smans{1}{kg}$.
	\item Do not adjust the knurled ring for the remainder of the experiment.
\end{enumerate}
...
Note that for the first several steps, you'll be keeping $r$ fixed.
It's up to you to choose whichever initial value for $r$ you'd like,
but we recommend starting with a fairly large $r$.
...
Note:  $H$ corresponds to the weight held by the hand -- 
that is, the weight hanging off the forearm bar.  
It does \emph{not} include the weight of the forearm, 
which is separately accounted for in $W$.  
($W$, in turn, does not include the weight held by the hand.)
...
\TODO{Consider swapping the order of 'varying model parameters' and 
'theoretical predictions' sections}
...
Note:  
If you need to find the angle $\theta$,
...
That is, \question{argue whether the formula given to you above
agrees or disagrees with the predictions you made before}.
